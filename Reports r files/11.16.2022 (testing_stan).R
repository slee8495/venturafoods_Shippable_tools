...
inv_lot_details <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/Shippable Tool Creation/test 8 - 11.16.22/Inventory Lot Detail - FG 11.16.22.xlsx")
...
custord <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/Shippable Tool Creation/test 8 - 11.16.22/MSTR custord - 11.16.22.xlsx")
...
planner_address <- read_excel("S:/Supply Chain Projects/Linda Liang/reference files/Address Book - 11.01.22.xlsx")
planner_address %>% 
  janitor::clean_names() %>% 
  readr::type_convert() %>% 
  dplyr::select(1:2) %>% 
...
exception_report <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/Shippable Tool Creation/test 8 - 11.16.22/exception report 11.16.22.xlsx")
...
iom_mbx <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/Shippable Tool Creation/test 7 - 11.10.22/SS Optimization by Location - Finished Goods November 2022.xlsx",
                      sheet = "Fin Goods")
...
# (Path revision Needed) Mfg reference ----
mfg <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/Shippable Tool Creation/test 8 - 11.16.22/Inventory Report for all locations - 11.16.22.xlsx")
...
mfg %>% 
  janitor::clean_names() %>% 
  dplyr::select(location, product_manufacturing_location, item) %>% 
...
fcst <- read_excel("S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/Demand Planning Team/BI Forecast Backup/DSX Forecast Backup - 2022.11.15.xlsx")
...
fcst %>% 
  ...
  dplyr::mutate(sku = gsub("-", "", sku),
                ref = paste0(location, "_", sku),
                mfg_ref = paste0(product_manufacturing_location_code, "_", sku),
...
fcst_pivot %>% 
  dplyr::select(1:9) -> fcst_pivot

fcst_pivot[, -3] -> fcst_pivot


# Add after custord
fcst_pivot -> fcst_pivot_2

colnames(fcst_pivot_2)[3] <- "current_month"
colnames(fcst_pivot_2)[4] <- "fcst_month_1"
colnames(fcst_pivot_2)[5] <- "fcst_month_2"
colnames(fcst_pivot_2)[6] <- "fcst_month_3"
colnames(fcst_pivot_2)[7] <- "fcst_month_4"
colnames(fcst_pivot_2)[8] <- "fcst_month_5"

fcst_pivot_2 %>% 
  dplyr::left_join(dplyr::select(custord_pivot, 1:2)) %>% 
  dplyr::mutate(sum_of_open_order_cases = as.integer(sum_of_open_order_cases),
                sum_of_open_order_cases = replace(sum_of_open_order_cases, is.na(sum_of_open_order_cases), 0)) %>% 
  dplyr::rename(custord_pivot = sum_of_open_order_cases) %>% 
  dplyr::mutate(current_month_after_custord = ifelse((current_month - custord_pivot) < 0, 0, current_month - custord_pivot),
                current_month_daily_avg = current_month_after_custord / 15,
                fcst_month_1_daily_avg = fcst_month_1 / 30,
                fcst_month_2_daily_avg = fcst_month_2 / 30,
                fcst_month_3_daily_avg = fcst_month_3 / 30,
                fcst_month_4_daily_avg = fcst_month_4 / 30,
                fcst_month_5_daily_avg = fcst_month_5 / 30) %>% 
...
first_day <- Sys.Date() + 15
colnames_fcst_pivot_last_month %>% data.frame() -> cfplm

cbind(first_day, cfplm) -> colnames_fcst_pivot
...
analysis_ref.2 %>% 
  filter(ref == "208_16451CVR") %>% 
  select(ref, dummy_ref, days_left_on_ssl, dummy_days_left_on_ssl, diff_factor, fcst_daily_avg_after_15_days)
